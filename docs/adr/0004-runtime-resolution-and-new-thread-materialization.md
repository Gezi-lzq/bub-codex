...
早期 spike 曾保留 `input_preview` 方便检查；正式实现已改为 `input_sha256` 与 `intent_sha256`，避免把完整用户任务或 materialized context 重复塞入每条 tape event。
...
- `input_preview` 只是 spike 便利字段；正式路径不再写入该字段，改用 `input_sha256` / `intent_sha256`。
...
- 如需检查完整 materialized input，应设计明确的 `input_ref` artifact，而不是恢复 `input_preview`。
