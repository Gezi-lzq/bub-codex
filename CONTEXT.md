...
- The runtime context boundary should only ensure a usable Codex thread context. Turn execution, stream normalization, and tape projection remain separate layers.
- `RuntimeContextKernel.ensure_thread_context()` composes `TapeStore`, `resolve_runtime_context`, Anchor creation, context materialization, Codex thread materialize/resume, and binding/failure tape events.
...
- `BubCodexRuntime.run_turn()` is the batch/reference ordinary user-turn facade. It reuses `RuntimeContextKernel.ensure_thread_context()`, runs a Codex turn, normalizes notifications into `CodexFact`, projects `codex.turn.started/completed` with `purpose=user_turn`, and reuses tool/side-effect projection.
...
- `src/bub_codex/plugin.py` now stays narrow as the Bub hook entrypoint. It exposes `BubCodexPlugin` / `create_plugin`, implements only `run_model_stream`, delegates comma commands to `state["_runtime_agent"].run(...)`, and delegates normal prompts to an injected `RuntimeStreamService`.
- `src/bub_codex/runtime_services.py` owns runtime service construction, lazy runtime caching, runtime cache keys, Bub/Republic tape-store selection, and the batch/reference `BubCodexRuntimeStreamService`.
...
- `BubCodexLiveRuntimeStreamService` is the MVP live notification bridge. It uses `RuntimeContextKernel.ensure_executable_context()`, consumes ordinary user-turn notification records as they arrive, appends projected tape events immediately in source order, keeps `phase=commentary` out of `StreamEvent("text")`, and emits `phase=final_answer` as Bub `text` and `final.text`.
...
- Runtime context resolution returns capability, not just startup status. `RuntimeContextKernel.ensure_executable_context()` returns `ExecutableContext | ContextUnavailable`, so live runtime can stop with the original materialization failure cause instead of inferring failure from a missing thread id.
- `RuntimeContextKernel` is the deep Module for Anchor/thread lifecycle. `BubCodexRuntime` keeps the batch/reference `run_turn()` facade, but executable-context resolution lives behind `RuntimeContextKernel.ensure_executable_context()`.
- Codex turn streaming is a resource lifecycle. `MaterializingCodexThreadService.start_turn_stream()` returns a `CodexTurnSession` with `records()` and `close()`, and live runtime closes it on success, error, or stream consumer cancellation.
- Compact Anchors preserve executable thread continuity by writing `codex.thread.bound(reason=compact_continuity)` to the same Codex thread. This keeps active thread resolution uniform: latest Anchor plus latest `codex.thread.bound` for that Anchor.
- New-thread materialization and binding projection lives in `new_thread_materialization.py`; compact projection lives in `compact_projection.py`; `tape_events.py` should stay a primitive event model, not a domain projection module.
- v0 live runtime is async at the Bub stream boundary but still uses a sync tape port. Async-only Republic tape stores fail fast until the planned async-first `RuntimeTape` port lands.
...
- [ADR 0008: Live Runtime Kernel Boundaries](docs/adr/0008-live-runtime-kernel-boundaries.md)
