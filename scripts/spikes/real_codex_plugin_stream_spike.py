...
async def main() -> None:
    ...
    workspace = Path("/tmp/bub-codex-real-plugin-workspace")
    workspace.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = ROOT / "artifacts/spikes" / f"real-codex-plugin-stream-{stamp}"
    ...
    with CodexClient(config=config) as client:
        ...
        result = await run_plugin_stream_once(
            stream_service,
            prompt="Reply exactly with: bub-codex-plugin-stream",
            session_id="real-plugin-session",
            state={"_runtime_workspace": str(workspace)},
            tape_store=store,
        )

    assert result.final_text == "bub-codex-plugin-stream"
    assert result.text == "bub-codex-plugin-stream"
    event_types = [event.type for event in result.tape_events]
    assert "codex.assistant_message.completed" in event_types
